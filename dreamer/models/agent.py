...
class AgentModel(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.value_model = DenseModel(
            feature_size + self.action_size, value_shape, value_layers, value_hidden, value_dist
        )
        self.posterior_model = ActionDecoder(
            output_size, feature_size + np.prod(value_shape), post_hidden, post_layers, post_dist
        )

        self.elite_model = DenseModel(
            feature_size, action_shape, elite_layers, elite_hidden, elite_dist
        )
    ...
    def policy(self, state: RSSMState):
        ...
        if self.action_dist == "tanh_normal" or self.action_dist == "beta":
