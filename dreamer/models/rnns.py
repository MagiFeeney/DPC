...
def drop_last(rssm_state: RSSMState):
    return RSSMState(
        rssm_state.mean[:-1],
        rssm_state.std[:-1],
        rssm_state.stoch[:-1],
...
def repeat_first_dim(rssm_state: RSSMState, repeat: int):
    repeats = (len(list(rssm_state.stoch.size())) + 1) * [1]
    repeats[0] *= repeat
    return RSSMState(
        rssm_state.mean.unsqueeze(0).repeat(repeats),
        rssm_state.std.unsqueeze(0).repeat(repeats),
        rssm_state.stoch.unsqueeze(0).repeat(repeats),
        rssm_state.deter.unsqueeze(0).repeat(repeats)
    )
...
class RSSMRollout(RollOutModule):
    ...
    def rollout_representation(
        ...
        # get the prior and post of the first time step
        priors = []
        posteriors = []
        
        mean, std = torch.chunk(self.transition_model._stochastic_prior_model(prev_state.deter), 2, dim=-1)
        std = tf.softplus(std) + 0.1
        dist = self.transition_model._dist(mean, std)
        stoch_state = dist.rsample()
        priors.append(RSSMState(mean, std, stoch_state, prev_state.deter))

        x = torch.cat([prev_state.deter, obs_embed[0]], -1)
        mean, std = torch.chunk(self.representation_model._stochastic_posterior_model(x), 2, dim=-1)
        std = tf.softplus(std) + 0.1
        dist = self.representation_model._dist(mean, std)
        stoch_state = dist.rsample()
        posteriors.append(RSSMState(mean, std, stoch_state, prev_state.deter))

        # get the prior and post of the remaining steps including the last step
        for t in range(steps):
            prior_state, posterior_state = self.representation_model(
                obs_embed[t + 1], action[t], prev_state
            )
