...
class ActionDecoder(nn.Module):
    ...
    def build_model(self):
        ...
        for i in range(1, self.layers):
            ...
            model += [self.activation()]
        if self.dist == "tanh_normal" or self.dist == "beta":
    ...
    def forward(self, *features):
        x = self.feedforward_model(torch.cat(features, -1))
        dist = None
        ...
        if self.dist == "beta":
            log_alpha, log_beta = torch.chunk(x, 2, -1)        
            alpha = F.softplus(log_alpha) + 1. + self.min_std
            beta  = F.softplus(log_beta) + 1. + self.min_std
